...
export default function Home() {
    ...
    const loadData = async () => {
        let response = await fetch("https://fooddeliveryapp-3f8s.onrender.com/api/foodData", {
            method: "POST",
            headers: {
                        ...
                        ? foodCat.map((data) => {
                            return (<div className='row mb-3'>
                                ...
                                {foodItem.length !== 0 ? foodItem.filter((item) => (item.CategoryName === data.CategoryName) && (item.name.toLowerCase().includes(search.toLocaleLowerCase())))
                                    .map(filterItems => {
                                        return (
                                            <div className="col-12 col-md-6 col-lg-3" key={filterItems._id}><Card foodItem={filterItems} options={filterItems.options[0]} ></Card> </div>
                                        )
